...
const CustomNodeTemplate = ({ nodeData }: { nodeData: any }) => {
    ...
    if (
        ...
        nodeData.name?.toLowerCase().includes('project')
    ) {
        return <ProjectNode nodeData={nodeData} />;
    }
    if (
        nodeData.type === 'sprint' ||
        nodeData.name?.toLowerCase().includes('sprint')
    ) {
        return <SprintNode nodeData={nodeData} />;
    }
    if (
        nodeData.type === 'task' ||
        nodeData.name?.toLowerCase().includes('task')
    ) {
